...
class SeriesSupervisor(base.Labeller):
    """
    ...
    multi_label: bool, optional
        If true, selecting a category will not automatically move to the next
        example, allowing you to add more than 1 label to a sample.
    ...
    def annotate(
        ...
        self.new_labels = self.labels.copy().astype(np.object)
    ...
    def _annotation_iterator(self):
        ...
        while len(self._label_queue) > 0:
            ...
            self._compose(row)
            offset, new_val = yield self
            
            if isinstance(self.new_labels, (pd.Series, pd.DataFrame)):
                row_labels = self.new_labels.loc[idx]
            else:
                row_labels = self.new_labels[idx]
            row_labels.append([offset, new_val])
    ...
    def _apply_annotation(self, sender):
        ...
        if isinstance(sender, dict) and "value" in sender and "offset" in sender:
            value = sender["value"]
            offset = sender["offset"]
        else:
            raise ValueError(
                "Keys don't work for series annotation."
            )
        self._annotation_loop.send(offset, value)
